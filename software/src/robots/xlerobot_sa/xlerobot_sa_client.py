...
from .config_xlerobot_sa import XLerobotSAConfig, XLerobotSAClientConfig


class XLerobotSAClient(Robot):
    config_class = XLerobotSAClientConfig
    name = "xlerobot_sa_client"

    def __init__(self, config: XLerobotSAClientConfig):
    ...
    def connect(self) -> None:
        ...
        if self._is_connected:
            raise DeviceAlreadyConnectedError(
                "XLerobotSA Daemon is already connected. Do not run `robot.connect()` twice."
            )
        ...
        if self.zmq_observation_socket not in socks or socks[self.zmq_observation_socket] != zmq.POLLIN:
            raise DeviceNotConnectedError("Timeout waiting for XLerobotSA Host to connect expired.")
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError("XLerobotSAClient is not connected. You need to run `robot.connect()`.")
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        """Command xlerobot_sa to move to a target joint configuration. Translates to motor space + sends over ZMQ
    ...
    def disconnect(self):
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError(
                "XLerobotSA is not connected. You need to run `robot.connect()` before disconnecting."
            )
