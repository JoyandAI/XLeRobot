...
from .config_xlerobot_sa import XLerobotSAConfig

logger = logging.getLogger(__name__)


class XLerobotSA(Robot):
    """
    The robot includes a three omniwheel mobile base and a single right arm.
    The leader arm is connected locally (on the laptop) and its joint positions are recorded and then
    forwarded to the remote follower arm (after applying a safety clamp).
    In parallel, keyboard teleoperation is used to generate raw velocity commands for the wheels.
    """

    config_class = XLerobotSAConfig
    name = "xlerobot_sa"

    def __init__(self, config: XLerobotSAConfig):
        ...
        # bus1 only has head motors (no left arm)
    ...
    def calibrate(self) -> None:
        ...
        homing_offsets = self.bus1.set_half_turn_homings(head_motors)
        ...
        range_mins, range_maxes = self.bus1.record_ranges_of_motion(head_motors)
        ...
        self.bus1.write_calibration(calibration_head)
        ...
        self.calibration = {**calibration_head, **calibration_right}
    ...
    def setup_motors(self) -> None:
        for motor in chain(reversed(self.head_motors)):
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        obs_dict = {**right_arm_state, **head_state, **base_vel}
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        """Command xlerobot_sa to move to a target joint configuration.
        ...
        if self.config.max_relative_target is not None:
            # Read present positions for right arm and head
            ...
            present_pos = {**present_pos_right, **present_pos_head}

            # Ensure safe goal position for each arm and head
            goal_present_pos = {
                key: (g_pos, present_pos[key]) for key, g_pos in chain(right_arm_pos.items(), head_pos.items())
            }
